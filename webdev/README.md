...
실제 개발하면서 플러그인을 직접 작성할 일은 거의 없다. 웹팩에서 제공하는 플러그인 혹은 써드파티 라이브러리를 찾아 사용한다.

자주 사용하는 플러그인은

bannerPlugin
-> 결과물에 빌드 정보 혹은 커밋 버전을 추가할 수 있다.

banner att에 문자열 혹은 함수 전달 가능
정보가 많다면 별도 파일로 뺀 후 (return : string), webpack.BannerPlungin($file)로 사용 가능

- 용례 : webpack.config.js 참고

DefinePlugin

개발환경과 운영환경으로 나뉘는 앱에 사용되는 환경정보를 다르게 쓸 수 있게 하는 플러그인

빌드 타임에 결정되는 값을 사용해야할 때 사용하면 된다
...
빌드 이전의 결과물을 제거하는 플러그인, 과거 파일이 남아있는 경우 먼저 싹 지우고 처리한다.

MiniCssExtractPlugin

번들결과에서 스타일시트 코드만 뽑아서 별도의 css 파일로 만드는 플러그인.

여러 개의 작은 파일로 만들어, 다운로드 속도가 빨라진다.
